...
$_SERVER['HTTP_ACCEPT_LANGUAGE'] = $_ENV['LANGUAGE'] ?: "en-us";

$vars = array(
  'name'      => getenv("SOPORTE_NAME")  ?: 'My Helpdesk',
  'email'     => getenv("SOPORTE_EMAIL") ?: 'helpdesk@example.com',
  'fname'       => getenv("SOPORTE_ADMIN_FIRSTNAME") ?: 'Admin',
  'lname'       => getenv("SOPORTE_ADMIN_LASTNAME")  ?: 'User',
  'admin_email' => getenv("SOPORTE_ADMIN_EMAIL")     ?: 'admin@example.com',
  'username'    => getenv("SOPORTE_ADMIN_USERNAME")  ?: 'ostadmin',
  'passwd'      => getenv("SOPORTE_ADMIN_PASSWORD")  ?: 'Admin1',
  'passwd2'     => getenv("SOPORTE_ADMIN_PASSWORD")  ?: 'Admin1',
  'prefix'   => getenv("SOPORTE_DATABASE_PREFIX")              ?: 'soporte_',
  'dbhost'   => getenv("SOPORTE_DATABASE_HOST")               ?: 'mysql',
  'dbname'   => getenv("SOPORTE_DATABASE_NAME")            ?: 'soporte',
  'dbuser'   => getenv("SOPORTE_DATABASE_USER")               ?: 'soporte',
  'dbpass'   => getenv("SOPORTE_DATABASE_PASSWORD")            ?: getenv("MYSQL_ENV_MYSQL_PASSWORD"),
  'smtp_host'       => getenv("SOPORTE_SMTP_HOST")            ?: 'localhost',
  'smtp_port'       => getenv("SOPORTE_SMTP_PORT")            ?: 25,
  'smtp_from'       => getenv("SOPORTE_SMTP_FROM"),
  'smtp_tls'        => getenv("SOPORTE_SMTP_TLS")             ?: 'on',
  'smtp_tls_certs'  => getenv("SOPORTE_SMTP_TLS_CERTS")       ?: '/etc/ssl/certs/ca-certificates.crt',
  'smtp_user'       => getenv("SOPORTE_SMTP_USER"),
  'smtp_pass'       => getenv("SOPORTE_SMTP_PASSWORD"),
  'cron_interval'   => getenv("CRON_INTERVAL")        ?: 5,
  'siri'     => getenv("INSTALL_SECRET"),
  'config'   => getenv("INSTALL_CONFIG") ?: '/data/upload/include/ost-config.php'
);
...
if (!$mailConfig = file_get_contents('/data/docker/conf/msmtp.conf')) {
...
if (!$linked) {
  ...
  if (!getenv("SOPORTE_DATABASE_HOST")) {
    err('Missing required environmental variable SOPORTE_DATABASE_HOST');
  }
  if (!getenv("SOPORTE_DATABASE_PASSWORD")) {
    err('Missing required environmental variable: SOPORTE_DATABASE_PASSWORD');
  }
} else {
...
DEFINE('SECRET_FILE','/tmp/secret.txt');
if (!$vars['siri']) {
...
#code to check if the database is installed and install it if it is not installed
#if (!$db_installed) {
